...
function Table() {
  ...
  const getDatas = async () => {
    try {
      const response = await axios.get('http://localhost:3000/supportEnquiry');
  ...
  const handleUpdate = async (orgId, updatedData) => {
    try {
      const response = await axios.put(`http://localhost:3000/supportEnquiry/${orgId}`, updatedData);
  ...
  const columns = [
      ...
      
    {
      ...
      cell: (row) => (
        ...
        <Button className='btn btn-3  mx-1' onClick={() => onDelete(row._id)}>
      ...
      <Datatable className='table-data-div'
        ...
        subHeaderComponent={
          <div className='table-top'>
            ...
            <div style={{ border: '1px solid #e8e8e8',padding: '6px 12px' , borderRadius:'5px',color: '#000000a3'}}>
                  {/* Add the user icon in place of the "Total Count" position *
                  <FontAwesomeIcon icon={faUser} style={{ marginRight: '5px' }} />
                  {/* Optionally, you can add a tooltip or title attribute *
